...
class XMLUtils:
    ...
    @staticmethod
    def parse_xml(xml_path: Path) -> Tuple[List[Tuple[int, int, int, int]], List[str]]:
        """
        解析XML文件，获取边界框和标签
    ...
    @staticmethod
    def get_xml_image_extension(xml_path: Path) -> str:
        """
        获取XML文件对应的图像文件扩展名

        Args:
            xml_path: XML文件路径(Path)

        Returns:
            图像文件扩展名（如 '.png'）
        """

        for img_suffix in DEFAULT_IMAGE_EXTENSIONS:
            if xml_path.with_suffix(img_suffix).exists():
                return img_suffix
    ...
    def parse_label(label: str) -> Tuple[str, int, int]:
        ...
        if len(parts) != 3:
            raise ValueError(
                f"Invalid label format: {label}, expected format is 'category_angle1_angle2'"
            )
    ...
    @staticmethod
    def get_xml_info(xml_path: Path) -> Dict[str, Any]:
        """
        获取XML文件的详细信息
        ...
        info = {"filename": None, "objects": [], "total_objects": 0}
    ...
    @staticmethod
    def validate_xml_structure(xml_path: Path | str) -> Dict[str, Any]:
        """
        验证XML文件结构
        ...
        for i, obj in enumerate(objects):
            ...
            if bbox is None:
                validation_result["errors"].append(
                    f"Object {i + 1} missing bounding box"
                )
                continue
                ...
                try:
                    ...
                    if value < 0:
                        validation_result["warnings"].append(
                            f"Object {i + 1} has negative {coord}: {value}"
                        )
                except ValueError:
                    validation_result["errors"].append(
                        f"Object {i + 1} has invalid {coord}: {coord_elem.text}"
                    )
            ...
            try:
                ...
                if xmin >= xmax:
                    validation_result["errors"].append(
                        f"Object {i + 1} has invalid x range: xmin={xmin}, xmax={xmax}"
                    )
                if ymin >= ymax:
                    validation_result["errors"].append(
                        f"Object {i + 1} has invalid y range: ymin={ymin}, ymax={ymax}"
                    )
            except (ValueError, TypeError):
                validation_result["errors"].append(
                    f"Object {i + 1} has invalid coordinate values"
                )
    ...
    @staticmethod
    def filter_objects_by_conditions(
        boxes: List[Tuple[int, int, int, int]],
        labels: List[str],
        conditions: Optional[Dict[str, Any]] = None,
    ) -> Tuple[List[int], List[Tuple[int, int, int, int]], List[str]]:
        """
        根据条件过滤对象
