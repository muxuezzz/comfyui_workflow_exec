...
__all__ = [
    "parse_xml",
    "get_xml_image_extension",
    "parse_label",
    "get_xml_info",
    "validate_xml_structure",
    "filter_objects_by_conditions",
]
...
def parse_label(label: str) -> tuple[str, int, int]:
    ...
    parts = label.split("_")
    if len(parts) != 3:
        raise ValueError(f"Invalid label format: {label}, expected format is 'category_angle1_angle2'")
...
def filter_objects_by_conditions(
    ...
    for i, (box, label) in enumerate(zip(boxes, labels, strict=False)):
        ...
        if "categories" in conditions:
            category, _, _ = parse_label(label)
