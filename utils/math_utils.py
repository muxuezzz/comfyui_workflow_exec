...
class MathUtils:
    ...
    def calculate_angle_difference(
        angle1: int, angle2: int, max_angle: int = 360
    ) -> int:
        """
        计算两个角度之间的最小差值
    ...
    @staticmethod
    def apply_angle_transformation(
        angle: int, offset: int, max_angle: int = 360
    ) -> int:
        """
        应用角度变换
        ...
        Returns:
            变换后的角度
        """
        return MathUtils.normalize_angle(angle + offset, max_angle)
    ...
    def calculate_iou(
        ...
        if (
            ...
            or intersection_ymax <= intersection_ymin
        ):
            return 0.0
    ...
    def is_overlapping(
        ...
        Returns:
            是否重叠
        """
        iou = MathUtils.calculate_iou(box1, box2)
    ...
    @staticmethod
    def randomize_value(
        base_value: Union[int, float], variation_percentage: float = 0.1
    ) -> Union[int, float]:
        """
        在基准值基础上随机化
    ...
    def generate_random_padding(
        min_padding: float = 0.1, max_padding: float = 1.5
    ) -> float:
        """
        生成随机内边距
